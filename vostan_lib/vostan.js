...
(function() {
    var VostanController = function(config) {
        ...
        var getExportedQueryData = function(rootID, queryID) {
            ...
            for(var i = 0; i < nodes.length; i++) {
                if(queryID == nodes[i].nodeID) {
                    return { nodes: nodes[i].query_nodes };
                }
            }
        };
        ...
        var initTheMap = function() {
            ...
            vostanApp.append('<div id="vostanAppSize">' + config.screen_w + 'x' + config.screen_h + '</div>')
            ...
            if (config.screen_w) {
                $("#vostanApp").css("width", config.screen_w).css("height", config.screen_h);
            }
        ...
        var loadTheMap = function(item) {
            ...
            $.ajax({
                type : 'GET',
                url : _host + '/map/root/' + root + '/lang/' + _lang + '/scale/' + _scale  + '?t=' + Date.now(),
                dataType : 'json',
                success : function(data) {
                    if(isValid(data)) {
        ...
        var EditController = function() {
            ...
            this.bindEnterForInput = function(wrapEl) {
                var tag = wrapEl.prop('tagName').toLowerCase();
                if (tag == 'textarea') {
                    return;
                }
                var el = tag == 'input' ? wrapEl : wrapEl.find('input');
                el.keyup(function(e) {
                    if (e.which == 13) {
                        $("#vostanEditStage").find('#edit-save').click();
                    }
                });
            };
            ...
            this.initVostanEditStage = function(width, height, title, wrapEl, offset) {
                ...
                _this.bindEnterForInput(wrapEl);
            ...
            this.initControlsClick = function() {
                ...
                $(".copysettings").bind("click", function(e) {
                    if (isEditMode()) {
                        ...
                        _node.copySettings(r);
                    }
                });
                $(".copy").bind("click", function(e) {
                    if (isEditMode()) {
                        if (localStorage) {
                            localStorage[document.location.host + "_copy"] = JSON.stringify(_node.attributes());
                        } else {
                ...
                $(".paste").bind("click", function(e) {
                    if (localStorage) {
                        window.vostan_tmpAttributes = JSON.parse(localStorage[document.location.host + "_copy"]);
                    }
                    if (window.vostan_tmpAttributes) {
                        _node.pasteAttributes(window.vostan_tmpAttributes);
                    }
                });
                $(".copyfull").bind("click", function(e) {
                    if (isEditMode()) {
                        Map.copyLayoutAttributes();
                    }
                });
                $(".pastefull").bind("click", function(e) {
                    if (isEditMode()) {
                        Map.pasteLayoutAttributes();
                    }
                });
            ...
            this.initControls = function(controlsNode) {
                controls = {
                    ...
                    copy_full: '<a class="copyfull">' + config.getMsg().btn.copy_settings_full + '</a>',
                    paste_full: '<a class="pastefull">' + config.getMsg().btn.paste_settings_full + '</a>',
                ...
                controlsNode.append(controls.clear_delimiter)
                            ...
                            .append(controls.copy_full)
                            .append(controls.paste_full);
        ...
        var Node = function(obj) {
            ...
            this.createAttributes = function(obj) {
                return {
                    ...
                    width : obj.width ? parseFloat(obj.width) : 90,
                    height : obj.height ? parseFloat(obj.height) : 40,
                    ...
                    titleTop : obj.titleTop ? parseFloat(obj.titleTop) : 0,
                    titleLeft : obj.titleLeft ? parseFloat(obj.titleLeft) : 0,
                    titleWidth : obj.titleWidth ? parseFloat(obj.titleWidth) : 90,
            ...
            var attributes = _this.createAttributes(obj);
            ...
            this.setNewAttributes = function(attr) {
                _this.oldAttr = attributes;
                attributes = _this.createAttributes(attr);
                _this.newAttr = attributes;
            };
            ...
            var updateInfo = function() {
                ...
                $.ajax({
                    ...
                    data : getUpdateData(),
                    //async : false,
                    //AB---changes-------for saving----------
                    beforeSend: function() {
                        var delta = $(document).width() / $("#vostanMap").width();
                        
                        if (attributes.imgInclude) {
                            attributes.imgHeight /= delta;
                            attributes.imgWidth /= delta;
                            attributes.imgTop /= delta;
                            attributes.imgLeft /= delta;
                        }
                       
                        // change node attributes
                        attributes.width /= delta;
                        attributes.height /= delta;
                        attributes.left /= delta;
                        attributes.top /= delta;
                       
                        if (attributes.titleInclude) {
                            attributes.titleTop /= delta;
                            attributes.titleLeft /= delta;
                            attributes.titleWidth /= delta;
                            attributes.titleHeight /= delta;
                        }

                        if (attributes.txtInclude) {
                            attributes.txtTop /= delta;
                            attributes.txtLeft /= delta;
                            attributes.txtWidth /= delta;
                            attributes.txtHeight /= delta;
                        }
            ...
            this.pasteAttributes = function(attr, force) {
            ...
            var getInterseptionAreas = function(pointer) {
                ...
                if (0 != areas.length) {
                    if (isFrameOverlapped(pointer)) {
                        areas.unshift(_this.$el.find('.view:first'));
                    }
                }
                return areas;
            };
            ...
            this.copySettings = function(updatePosition) {
                $.ajax({
                    type : 'GET',
                    url : _host + '/copysettings/node/' + attributes.nodeID + '/root/' + _root + '/updateposition/' + updatePosition,
                    dataType : 'json',
                    success : function(data) {
            ...
            this.addChildNode = function(_lang) {
                var node = JSON.parse(getNewData(_lang));
                Map.appendNewNode(node, "add", attributes.nodeID);
            };
            ...
            this.addNewNode = function(nodeID) {
                var rootID = _root;
                if (nodeID) {
                    rootID = nodeID;
                }
                $.ajax({
                    type : 'POST',
                    url : _host + '/add/node/' + rootID + '/root/' + rootID + '/lang/' + _lang,
                    dataType : 'json',
                    contentType : 'application/json',
                    data : getUpdateData(),
                    success : function(data) {
                        if (isValid(data) && data.node) {
                            ...
                            var linkAttr = {};
                            if (!nodeID) {
                                linkAttr.nodeID = node.nodeID;
                                linkAttr.linkedNodeID = _root;
                            } else {
                                _this.hideFromPage(nodeID, data.node);
                                _this.appendNewNode(data.node, node);
                                linkAttr.nodeID = data.node;
                                linkAttr.linkedNodeID = nodeID;
                            }
            ...
            this.appendNewNode = function(nodeID, newNode) {
                var rootID = attributes.nodeID;
                var nodeData = getUpdateData();
                if (nodeID && newNode) {
                    rootID = nodeID;
                    nodeData = newNode;
                }
                $.ajax({
                    type : 'POST',
                    url : _host + '/append/node/' + rootID + '/root/' + _root,
                    dataType : 'json',
                    contentType : 'application/json',
                    data : nodeData,
                    success : function(data) {
            ...
            this.hideFromPage = function(root, node) {
                var rootID = _root;
                var nodeID = attributes.nodeID;
                if (root && node) {
                    rootID = root;
                    nodeID = node;
                }
                $.ajax({
                    type : 'GET',
                    url : _host + '/hide/node/' + nodeID + '/root/' + rootID,
                    dataType : 'json',
                    success : function(data) {
                        if (isValid(data)) {
            ...
            this.editNodeContent = function(selector) {
                ...
                selector.resizable({
                    ...
                    resize : function(event, ui) {
                        ...
                        selector.find(".params").text("w:" + selector.innerWidth() + " h:" + selector.innerHeight());
            ...
            this.isAudio = function() {
                return (attributes.tags.length > 0) && (attributes.tags.split(",").indexOf("audio") >= 0);
            };
            ...
            this.render = function(rootAttr) {
                ...
                if (this.isAttachment()) {
                    ...
                    node.append(view);
                    this.$el = node;
                } else if (this.isVideo() || this.isPDF() || this.isAudio()) {
                    view.append(exp)
                        .append(tags)
                        .append(img)
                        .append(info)
            ...
            this.setQueryNodeData = function() {
                ...
                if (linkedNodeID > 0) {
                    if (isExport()) {
                        var nodes = getExportedQueryData(_this.isRoot() ? _root : linkedNodeID,
                                _this.attributes().nodeID);
            ...
            this.drawQueryResult = function(queryNode, data) {
                ...
                for (var i = 0; i < data.nodes.length; i++) {
                    ...
                    if (data.nodes[i].tags.indexOf("video") > -1 ||
                        data.nodes[i].tags.indexOf("audio") > -1) {
                            data.nodes[i].img = data.nodes[i].img.split(",")[1];
                    }
                    if (data.nodes[i].img &&
            ...
            this.setElAttributes = function(el, isCopy) {
                el.removeClass("node-n node-ni node-nit node-nt node-it node-i node-t node-i node-proot node-root");
                ...
                if (this.isQuery()) {
                    var tmp = tags.split(",vostan::query")[0];
                    tmp = tmp.indexOf("vostan::query") == -1 ? tmp : "";
                    el.find(".tags:first").html("").append("<span>" + tmp + "</span><a><span class='tag-icon'></span><span class='tag-info'>" + tags + "</span></a>");
                } else {
                ...
                if (attributes.titleInclude) {
                    ...
                    if (attributes.titleTop > (attributes.imgTop + attributes.imgHeight) || !attributes.imgInclude) {
                        el.find(".info:first .title-box").css('text-align', "center");
                    }
                ...
                if (el.find(".title-box").height() !== 0 && !this.isQuery() && !isCopy) {
                    attributes.titleHeight = el.find(".title-box").height();
                }

                //AB......changes.......... window(width,height)
                var delta = $(document).width() / $("#vostanMap").width();
	        	if (attributes.imgInclude) {
                    attributes.imgHeight *= delta;
                    attributes.imgWidth *= delta;
               ...
               if (attributes.txtInclude) {
                ...
                var nd = $("node#" + attributes.nodeID + " .info div");
                var fontSize = $(nd).css("font-size");
                fontSize *= delta;

                $(nd).css("font-size", fontSize);
	
                //---------------------

                if (attributes.imgInclude) {
                    if (!this.isVideo() && ! this.isPDF() && ! this.isAudio()) {
            ...
            this.setElTextAttrs = function(el) {
                ...
                if (attributes.imgInclude && (this.isVideo() || this.isPDF() || this.isAudio())) {
                ...
                if (isEditMode()) {
                    ...
                    this.$el.resizable({
                        ...
                        resize : function(event, ui) {
                                     $("#editControls,.link-edits").hide();
                                     $(this).find('.view').find('.params').appendTo($(this));
                                     $(this).find(".params").text("w:" + $(this).innerWidth() + " h:" + $(this).innerHeight());
                                     $(this).find(".params").css("display", "block");
                    ...
                    if (_this.$el.find(".ui-resizable-handle").css("display") == "block") {
                        destroyDraggablesResizables();
                        Map.updateNodesClickedStep(attributes.nodeID);
                    }
                } else {
        ...
        var MapController = function() {
            ...
            var addNodes = function() {
                var tmp = [];
                var newNodeStart = nodeClicked ? nodeClicked.attributes() :
                    newRootAttributes;
                for (var i = 0; i < _nodes.length; i++) {
                    var node = nodeByID(_nodes[i].nodeID);
                    if (node) {
                        node.setNewAttributes(_nodes[i]);
                    } else {
                        var new_node = createNode(_nodes[i]);
                        new_node.newAttr = newNodeStart;
                        tmp.push(new_node);
                        new_node.render(new_node.newAttr);
                    }
                }
                nodes.concat(tmp);
                if (nodeClicked) {
            ...
            var showNodes = function() {
                nodes = nodes.filter(function(node) {
                    if (! node.newAttr && node.oldAttr) {
                        node.hide(newRootAttributes);
                        return false;
                    } else {
                        //node.setElAttributes(node.$el);
                        node.show();
                        return true;
                    }
                });
            ...
            var getPointsAttributes = function(id1, id2) {
                // runs different algoritms for rendering links
                if (config.draw_link == "projections") {
                    return getPointsAttributes_projections(id1, id2);
                }
                return getPointsAttributes_midpoints(id1, id2);
            }

            /**
             * @brief - Finds points on the edges of the two specified nodes
             * for conenct them with a link
             *
             * @detail - takes the line passing through the centers of the nodes and finds
             * the cross points with the node edges nearer to the other node
             */
            var getPointsAttributes_midpoints = function(id1, id2) {
             ...
             * for conenct them with a link
             *
             * @detail - takes the projections of the nodes on each of the x and y axises.
             * For each axis if the projections intersect then the midpoint of
             * the intersection is taken, otherwise the nearest points to other
             * node projections are taken
             */
            var getPointsAttributes_projections = function(id1, id2) {
                var top1 = nodeByID(id1).attributes().top;
                //top of node1
                var left1 = nodeByID(id1).attributes().left;
                //left of node1
            ...
            var get_middle_point = function(a, b, c, d) {
                return (Math.max(Math.min(a, b), c) +
                        Math.min(Math.max(c, d), a)) / 2;
            }

            var link1stPoint_x = get_middle_point(left1 + width1, left2 + width2, left1, left2);
            var link1stPoint_y = get_middle_point(top1 + height1, top2 + height2, top1, top2);
            var link2ndPoint_x = get_middle_point(left2 + width2, left1 + width1, left2, left1);
            var link2ndPoint_y = get_middle_point(top2 + height2, top1 + height1, top2, top1);


                return [link1stPoint_x, link1stPoint_y, link2ndPoint_x, link2ndPoint_y];
            };
            ...
            this.copyLayoutAttributes = function() {
                window.vostan_sourceLayout = {};
                window.vostan_sourceLayout.root = _root;
                window.vostan_sourceLayout.append = {};
                window.vostan_sourceLayout.links = [];
                for (var i in nodes) {
                    var tag = nodes[i].attributes().tags.replace(",vostan::template", "");
                    var nodeID = nodes[i].attributes().nodeID;
                    if (tag && tag.length > 0 && tag.indexOf("vostan::action") == -1) {
                        window.vostan_sourceLayout[tag] = nodes[i].attributes();
                    } else {
                        window.vostan_sourceLayout.append[nodeID] = nodes[i].attributes();
                    }
                }
                for (var i in links) {
                    var link = links[i].attributes();
                    if ((link.nodeID == _root && window.vostan_sourceLayout.append[link.linkedNodeID]) ||
                        (link.linkedNodeID == _root && window.vostan_sourceLayout.append[link.nodeID])) {
                        window.vostan_sourceLayout.links.push(links[i].attributes());
                    }
                }
                localStorage[document.location.host + "_copy_full"] = JSON.stringify(window.vostan_sourceLayout);
            };

            this.pasteLayoutAttributes = function() {
                if (localStorage) {
                    window.vostan_sourceLayout = JSON.parse(localStorage[document.location.host + "_copy_full"]);
                }
                if (window.vostan_sourceLayout) {
                    for (var i in nodes) {
                        var tag = nodes[i].attributes().tags;
                        var nodeID = nodes[i].attributes().nodeID;
                        if (tag && tag.length > 0 &&
                            tag.indexOf("vostan::action") == -1 &&
                            window.vostan_sourceLayout[tag]) {
                            nodes[i].pasteAttributes(window.vostan_sourceLayout[tag], true);
                        } else if (window.vostan_sourceLayout.append[nodeID]) {
                            nodes[i].pasteAttributes(window.vostan_sourceLayout.append[nodeID], true);
                        }
                    }

                    var appendNodes = function(counter) {
                        var nodes = Object.keys(window.vostan_sourceLayout.append);
                        if (counter == nodes.length) {
                            appendLinks(0);
                        } else {
                            var nodeID = nodes[counter];
                            $.ajax({
                                type : 'GET',
                                url : _host + '/paste/node/' + nodeID + '/from/' + window.vostan_sourceLayout.root + '/to/' + _root,
                                dataType : 'json',
                                contentType : 'application/json',
                                success : function(data) {
                                    if (isValid(data)) {
                                        Map.appendNewNode(window.vostan_sourceLayout.append[nodeID], "paste");
                                    } ;
                                },
                                complete: function() {
                                    appendNodes(++counter);
                    ...
                    var appendLinks = function(counter) {
                        var links = window.vostan_sourceLayout.links;
                        if (counter == links.length) {
                            Map.redrawLinks(_root);
                        } else {
                            var add = false;
                            if (links[counter].nodeID == window.vostan_sourceLayout.root) {
                                links[counter].nodeID = _root;
                                add = true;
                            } else if (links[counter].linkedNodeID == window.vostan_sourceLayout.root) {
                                links[counter].linkedNodeID = _root;
                                add = true;
                            }
                            if (add) {
                                $.ajax({
                                    type : 'POST',
                                    url : _host + '/add/link/root/' + _root + '/lang/en',
                                    dataType : 'json',
                                    contentType : 'application/json',
            ...
            this.appendNewNode = function(item, mode, nodeID) {
                ...
                if (mode == "append") {
                    ...
                    newNode.attributes().users = nodeByID(rootID).attributes().users;
                    newNode.attributes().viewers = nodeByID(rootID).attributes().viewers;
                    newNode.renderInEditMode();
                    newNode.addNewNode(nodeID);
                } else if (mode == "show") {
                    newNode.attributes().user = "editor";
