...
//GetMap by ID
$app -> get('/map/root/:rid/lang/:lang/scale/:scale', 'getMapWithScale');
//GetMap by ID
$app -> get('/map/root/:rid/lang/:lang/scale/:scale/nohtml', 'getMapWithScalePlain');
//GetMap by ID
...
$app -> get('/copysettings/node/:nid/root/:rid/updateposition/:updateposition', 'copySettings');
...
function getMap($rid, $lang) {
    echoResponse(getMapData($rid, $lang, 1, false, false));
}

function getMapWithScale($rid, $lang, $scale) {
    echoResponse(getMapData($rid, $lang, $scale, false, false));
}

function getMapWithScalePlain($rid, $lang, $scale) {
    echoResponse(getMapData($rid, $lang, $scale, false, true));
}
...
function getMapData($rid, $lang, $scale, $isExport, $escapehtml) {
    try {
        $db = getConnection();
        $user_condition = "";
        $username = getSessionUser();
        ...
        $sql1 = "SELECT 
                ...
                n.location AS 'location',
                s.top * $scale AS 'top',
                s.left * $scale AS 'left',
                s.width * $scale AS 'width',
                s.height * $scale AS 'height',
                s.imgWidth * $scale AS 'imgWidth',
                s.imgHeight * $scale AS 'imgHeight',
                s.imgLeft * $scale AS 'imgLeft',
                s.imgTop * $scale AS 'imgTop',
                s.titleWidth * $scale AS 'titleWidth',
                s.titleHeight * $scale AS 'titleHeight',
                s.titleLeft * $scale AS 'titleLeft',
                s.titleTop * $scale AS 'titleTop',
                s.txtWidth * $scale AS 'txtWidth',
                s.txtHeight * $scale AS 'txtHeight',
                s.txtLeft * $scale AS 'txtLeft',
                s.txtTop * $scale AS 'txtTop',
        ...
        $sql2 = "SELECT
                 ...
                 l.tags AS 'linkTags_en',
                 l.tags_hy AS 'linkTags_hy',
                 l.tags_ru AS 'linkTags_ru',
        ...
        foreach ($node as $key => $value) {
            ...
            if ($escapehtml) {
                $value -> txt = strip_tags($value -> txt, '');
                $value -> txt_hy = strip_tags($value -> txt_hy, '');
                $value -> txt_en = strip_tags($value -> txt_en, '');
                $value -> txt_ru = strip_tags($value -> txt_ru, '');
                $value -> defaultTxt = strip_tags($value -> defaultTxt, '');
            }
            $value -> user = "";
            if ($isExport) {
                ...
                if (false !== strpos($value -> tags, 'vostan::query')) {
                    $value->query_nodes = array();
                    foreach ($links as $key => $lvalue) {
                        if ($lvalue -> nodeID == $value -> nodeID) {
                            $value->query_nodes = getQueryHelper($value -> tags, $lang, $lvalue -> linkedNodeID);
                        }
                    }
                }
            } else if (isUserSet()) {
...
function createTables($db) {
    ...
    $sql3 = "CREATE TABLE settings (
        ...
        width double NOT NULL DEFAULT '90',
        height double NOT NULL DEFAULT '40',
...
function exportPresentation($rid, $lang) {
    ...
    foreach ($input as $key => $value) {
        $data .= '"' . $value . '" : ' . getMapData($value, $lang, 1, true, false);
...
function appendNode($nid, $rid) {
    ...
    try {
        ...
        $sql1 = "INSERT INTO settings (nodeID, linkedNodeID, top, left, width, height, titleTop, titleLeft, titleWidth, titleHeight, modified)
             VALUES (:rid, :nid, 10, 10, 90, 40, 0, 0, 90, 40, date());";
        $stmt = $db -> prepare($sql1);
        $stmt -> bindParam("rid", $rid);
...
function copySettings($nid, $rid, $updateposition) {
    checkSession();
    // get and decode JSON request body
    $request = \Slim\Slim::getInstance() -> request();
    $body = $request -> getBody();
	...
	$sql2 .= " WHERE linkedNodeID = :nid AND nodeID != :nid ;";
	...
	if ("true" === $updateposition) {
        ...
        echoResponse('{"error":{"copySettings":"' . $e -> getMessage() . '"}}');
    }
}
...
function addNode($nid, $rid, $lang) {
    ...
    try {
        ...
        $sql3 = "INSERT INTO settings (nodeID, linkedNodeID, top, left, width, height, titleTop, titleLeft, titleWidth, titleHeight, modified,titleInclude, imgInclude ,txtInclude, leaf)
                 VALUES  ($max_id, $max_id, 200, 200, 90, 40, 0, 0, 90, 40, date(), :titleInclude, :imgInclude, :txtInclude, :leaf);";
        $stmt = $db -> prepare($sql3);
        $stmt -> bindParam("titleInclude", $input -> titleInclude);
        $stmt -> bindParam("imgInclude", $input -> imgInclude);
        $stmt -> bindParam("txtInclude", $input -> txtInclude);
        ...
        if ($rid) {
        $sql4 = "INSERT INTO settings (nodeID, linkedNodeID, top, left, width, height, titleTop, titleLeft, titleWidth, titleHeight, modified, titleInclude, imgInclude ,txtInclude, leaf)
                 VALUES ($max_id, :rid, 10, 10, 90, 40, 0, 0, 90, 40, date(), :titleInclude, :imgInclude, :txtInclude, :leaf);";
        $stmt = $db -> prepare($sql4);
        $stmt -> bindParam("titleInclude", $input -> titleInclude);
        $stmt -> bindParam("imgInclude", $input -> imgInclude);
        $stmt -> bindParam("txtInclude", $input -> txtInclude);
        ...
        $sql5 = "INSERT INTO settings (nodeID, linkedNodeID, top, left, width, height, titleTop, titleLeft, titleWidth, titleHeight, modified, titleInclude, imgInclude ,txtInclude, leaf, carousel) 
                VALUES (:rid, $max_id, :top, :left, :width, :height, :titleTop, :titleLeft, :titleWidth, :titleHeight, date(), :titleInclude, :imgInclude, :txtInclude, :leaf,:carousel);";
        $stmt = $db -> prepare($sql5);
        $stmt -> bindParam("titleInclude", $input -> titleInclude);
        $stmt -> bindParam("imgInclude", $input -> imgInclude);
        $stmt -> bindParam("txtInclude", $input -> txtInclude);
